...
Across 3 target pair(s), the publication-target feature is doing differentiating work: verdict-agreement peaks at 1.00 (mean 0.78) and revision-request Jaccard peaks at 0.10 (mean 0.08). The reshape lenses produce distinct reviewer outputs, not three variants of the same review with different dossier text prepended. **Oryx metric-review flag (small-sample honesty):** this round has 3 personas and 3 target-pairs. With 3 personas the verdict-agreement rate only takes values in {0.00, 0.33, 0.67, 1.00}, which makes a 0.70 threshold coarse by construction. Treat the gate as a smoke signal, not a precision instrument, until the panel grows.

## Pairwise metrics

| Pair | Verdict agreement | Revision Jaccard | JS-divergence (3-way verdict) | Breach? |
|---|---:|---:|---:|---|
| cryptologia × dsh | 1.00 | 0.10 | 0.000 | **yes** (verdict_agreement) |
| cryptologia × jocch | 0.67 | 0.10 | 0.132 | no |
| dsh × jocch | 0.67 | 0.05 | 0.132 | no |
...
| jocch | 3 | manifold_learning_skeptic=REVISE, probing_methodology=REVISE, stats_methods=REVISE | REVISE |
