...
\usepackage{setspace}
% Overfull hbox mitigation for prose with many long URLs / texttt
% paths. \emergencystretch lets TeX widen inter-word spacing up to
% 3em when a line can't otherwise be broken — sufficient to absorb
% every overfull we see without visibly affecting justification.
\setlength{\emergencystretch}{3em}
% \sloppy relaxes line-breaking penalties for this document. Combined
% with the emergencystretch above, this eliminates protruding URLs
% without needing per-paragraph \begin{sloppypar} wrappers.
\sloppy
...
semantic analysis of the manuscript --- a \emph{distant-viewing} case
study that treats its 206 page images as the primary signal and asks
what the visual channel yields when the text channel is inaccessible. Using a zero-shot visual semantic profiling platform
...
Manuscript carry thematic structure that is computationally
recoverable even though the underlying text is not, and that the
scholarly section structure is independently recoverable by a
non-textual method. This is a claim about the computational
accessibility of section structure, not a claim about the
manuscript's meaning in any broader humanistic sense. We do not
claim to have read the manuscript. We claim only
